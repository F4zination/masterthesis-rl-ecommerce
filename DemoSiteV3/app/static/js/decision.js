...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        slots.forEach((slot, index) => {
            ...
            const opportunityId = `${decisionPoint}:${window.location.pathname}:${index}`;
            requestSlotDecision(slot, decisionPoint, context, opportunityId, 0);
        });
    });

    async function requestSlotDecision(slot, decisionPoint, context, opportunityId, opportunityIndex) {
        const elapsedMs = Date.now() - PAGE_START_TS;
        const requestContext = {
            ...context,
            opportunity_id: opportunityId,
            opportunity_type: 'slot',
            opportunity_index: opportunityIndex,
            elapsed_ms: elapsedMs,
        };

        trackOpportunityEvent('opportunity_checked', decisionPoint, requestContext);
        ...
        try {
            ...
            if (!res.ok) {
                trackOpportunityEvent('opportunity_error', decisionPoint, {
                    ...requestContext,
            ...
            if (data.should_show === false || data.action === 'no-op') {
                trackOpportunityEvent('opportunity_skipped', decisionPoint, {
                    ...requestContext,
                    reason_code: data.reason_code || 'no_op',
                    next_check_after_ms: data.next_check_after_ms || 0,
                });
                const deferMs = Number(data.next_check_after_ms || 0);
                if (deferMs > 0 && opportunityIndex < MAX_DEFER_RETRIES) {
                    setTimeout(() => {
                        requestSlotDecision(
                            slot,
                            decisionPoint,
            ...
            trackOpportunityEvent('opportunity_served', decisionPoint, {
                ...requestContext,
                decision_id: data.decision_id,
                action: data.action,
            });
            ...
            trackOpportunityEvent('opportunity_error', decisionPoint, {
                ...requestContext,
                reason_code: 'network_or_parse_error',
                http_status: 0,
            });
        }
    }
    ...
    function trackOpportunityEvent(eventType, decisionPoint, metadata) {
        fetch('/api/events', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
    ...
    async function requestScrollDecision(pageDepth) {
        const elapsedMs = Date.now() - PAGE_START_TS;
        const opportunityId = `scroll:${window.location.pathname}:70`;
        const context = {
            page_depth: pageDepth,
        ...
        try {
            ...
            if (!res.ok) {
                trackOpportunityEvent('opportunity_error', 'scroll_engagement', {
                    ...context,
                    reason_code: 'http_error',
                    http_status: res.status,
                });
            ...
            const data = await res.json();
            if (data.should_show === false || data.action === 'no-op') {
                trackOpportunityEvent('opportunity_skipped', 'scroll_engagement', {
                    ...context,
                    reason_code: data.reason_code || 'no_op',
                });
                return;
            ...
            if (data.action === 'help_popup') {
                trackOpportunityEvent('opportunity_served', 'scroll_engagement', {
                    ...context,
                    decision_id: data.decision_id,
                    action: data.action,
                });
